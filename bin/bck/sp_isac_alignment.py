...
def ali2d_single_iter_CLEAN(
    ...
    """
    Cleaned up version of ali2d_single_iter() (see above) with only those 
    branches implemented that are being used by ISAC.
    """
    ...
    for im in range(len(data)):
        ...
        # transform coordinates and get shifts used in ormq
        ...
        _, sxi, syi, _               = inverse_transform2(alpha, sx, sy)
        ...
        # The search range procedure was adjusted for 3D searches, so since in 2D the order of operations is inverted, we have to invert ranges (PAP)
        txrng = search_range(nx, ou, sxi, xrng, "ali2d_single_iter")
        tyrng = search_range(ny, ou, syi, yrng, "ali2d_single_iter")
        txrng = [txrng[1],txrng[0]]
        tyrng = [tyrng[1],tyrng[0]]

        # align current image (im) to the polar'd reference (cimage)
        [angt, sxst, syst, mirrort, peakt] = ormq_CLEAN(ima, cimage, txrng, tyrng, step, mode, numr, cnx+sxi, cny+syi, delta)

        # combine parameters and write them into the image header (ignore previous angle and mirror)
