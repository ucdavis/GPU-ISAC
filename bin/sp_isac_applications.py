...
def get_c_ptr_array( emdata_list ):
    ...
    for img in emdata_list:
        ...
        assert img_np.dtype == numpy.float32
...
def get_tex_limit( img_dim, cuda_device_id=0 ):
    return cu_module.get_texture_limit( ctypes.c_int(img_dim), ctypes.c_int(cuda_device_id) )
...
def ali2d_base_gpu_isac_CLEAN(
    ...
    """
    NOTE: CTF filtering is NOT performed anymore, regardless of CTF flag. This
    is because GPU ISAC now does that during the initial data read. If the CTF
    flag is set, then filt_ctf() has already been applied to the data!
    ...
    # set alignment search ranges (ISAC hardcodes xrng==yrng)
    yrng = xrng = sp_utilities.get_input_from_string(xr)
    step = sp_utilities.get_input_from_string(ts)
    ...
    nx = sp_utilities.bcast_number_to_all(nx, source_node=main_node, mpi_comm=mpi_comm)

    if CTF:
        ctf_app = sp_utilities.bcast_number_to_all(ctf_app, source_node=main_node, mpi_comm=mpi_comm)
    ...
    aln_cfg = AlignConfig(
        ...
        "pre_aln".encode("utf-8"))   # alignment type
    aln_cfg.freeze()
    ...
    gpu_batch_count = int(len(data)/gpu_batch_limit) if len(data)%gpu_batch_limit==0 else int(len(data)//gpu_batch_limit+1)
    ...
    if( gpu_batch_count == 1 ):
        cu_module.pre_align_fetch(
            ...
            ctypes.c_char_p("sbj_batch".encode("utf-8")) )
    ...
    for N_step in range(len(xrng)):
        ...
        for Iter in range(max_iter):
            ...
            sp_utilities.reduce_EMData_to_root(ave1, myid, main_node, comm=mpi_comm)     # sum_oe returns sum of [o]dd and [e]ven items
            sp_utilities.reduce_EMData_to_root(ave2, myid, main_node, comm=mpi_comm)
            ...
            else: tavg =  sp_utilities.model_blank(nx, nx)
            del ave1, ave2
            ...
            sp_utilities.bcast_EMData_to_all(tavg, myid, main_node, comm=mpi_comm)
            cs = mpi.mpi_bcast(cs, 2, mpi.MPI_FLOAT, main_node, mpi_comm)
            cs = list(map(float, cs))

            if total_iter != max_iter*len(xrng): # NOTE: total_iter is incremented at the start of the loop, the last iteration does not do an alignment
                ...
                ####################################### CPU
                """
                sx_sum, sy_sum, nope = alignment.ali2d_single_iter( # ISAC hardcoded
                    data,               # img data
                    numr,               # no. of rings
                    wr,                 # ring weights              gpu version doesn't use these
                    cs,                 #                           [0.0, 0.0]
                    tavg,               # target average
                    cnx,                # center_x
                    cny,                # center_y
                    xrng[N_step],       # search range              [28, 12, 4]
                    yrng[N_step],       # search range              [28, 12, 4]
                    step[N_step],       # search step               [ 7,  3, 1]
                    nomirror=nomirror,  # no-mirror flag            False
                    mode=mode,          # full circle thing
                    CTF=CTF,            # CTF flag                  False           <-- wat?
                    delta=delta,        # 90 degree reset thing     90.0
                    random_method=random_method) #                  ""
                ...
                sys.stdout.write( "\r--- %d/%d - %d/%d (#%d)" % (N_step, len(xrng), Iter, max_iter, total_iter) )
                sys.stdout.flush()
                """
                """
                ali2d_single_iter_CLEAN(data, tavg, search_range):
                + convert tavg to polar
                + for im in data:
                    + ormq(im, tavg, search_range)
                    + update im header w/ new aln_params
                + return sum of all x/y shifts (x-shift negated in case of mirroring)
                """
                ####################################### CPU
                ###########################################
                ####################################### GPU
                ...
                if Iter < 5 and filament_width is not None:
                    n = filament_width // 2
                    cylin_np = numpy.zeros((nx, nx))
                    cylin_np[int(nx//2 - n):int(nx//2 + n)] = 1
                    tavg = sp_utilities.numpy2em_python(cylin_np)
                cu_module.pre_align_fetch(
                    get_c_ptr_array([tavg]),
                    ctypes.c_int(1),
                    ctypes.c_char_p("ref_batch".encode("utf-8")) )  # NOTE: happens for each shift change
                ...
                for gpu_batch_idx in range(gpu_batch_count):
                    ...
                    # print progress bar
                    gpu_calls_ttl = (len(xrng) * max_iter - 1) * gpu_batch_count
                ...
                for k, img in enumerate(data):
                    ...
                    sp_utilities.set_params2D( img, [angle, shift_x, shift_y, gpu_aln_param[k].mirror, 1.0], "xform.align2d" )
                ...
                if myid == main_node:
                    ...
                    if mirror_consistent!=0:
                        ...
                        region, hist = sp_statistics.hist_list(pixel_error_list, 20)
                        ###############################for p in range(20): log.add("      %14.2f: %6d"%(region[p], hist[p]))
                    ###############################log.add("\n\n")
    ...
    if myid==0: print( time.strftime("%Y-%m-%d %H:%M:%S :: ", time.localtime()) + "ali2d_base_gpu_isac_CLEAN() :: Write headers" )
...
def within_group_refinement_CLEAN(
    data,
    maskfile,
    randomize,
    first_ring,
    ...
    cnt  = data[0].get_xsize()/2+1
    mode = "F"
    numr = sp_alignment.Numrinit(first_ring, last_ring, rstep, mode)
    wr   = sp_alignment.ringwe(numr, mode)

    cdata = [ data[i] for i in range(len(data)) ]
    ...
    for i in range(max_iter):
        ...
        if( FH > 0.0):
            fl = 0.1 + (FH-0.1)*i / float(max_iter-1)
            tavg = sp_filter.filt_tanl(tavg, fl, FF)
        
        if i%4 != 0 or i >= max_iter-10:
            delta = 0.0
        else:
            delta = dst

        sp_alignment.ali2d_single_iter_CLEAN(
            cdata,
            numr,
            wr,
            [0,0],
...
def multigroup_refinement_gpu_fit_max(
    max_class_num,
    max_class_size,
    img_dim,
    outer_polar_radius,
    ...
    """
    The whole idea behind ISAC is that clusters are not allowed to grow over a
    given size limit. Usually this means that classes have a variable number of
    members that is below this limit, i.e., class size is heterogeneous.
    
       In order to determine a data volume that can be guaranteed to fit on the
    card this function works just like multigroup_refinement_gpu_fit() except
    that each class is assumed to have full size. Note that we only need to
    compute this number once and can re-use it for all future refinements of
    the current ISAC execution.

       This means that some GPU resources are guaranteed to go to waste since
    we are processing less data than the GPU would be able to fit. However, if
    we would like to have full GPU exploitation at any time we would have to
    run multigroup_refinement_gpu_fit() again for each computation to determine
    exactly how much data we can fit this time. Since the memory estimation
    function takes some time this would most likely introduce more overhead
    costs than we would be getting out of it, so fuck it.

       NOTE: Once multigroup_refinement_gpu_fit() performs a binary search it
    might be fast enough to use many times without taking a performance hit.

    Args:
        max_class_num (int): Upper limit for the possible number of classes
            given the current parameters. Equals particle_count/min_class_size.

        max_class_size (int): Maximum allowed class size (aka "img_per_group",
            aka "maximum_group_size"). This function will assume all classes
            have this size for the purpose of determining the GPU fit.

        outer_polar_radius (int): ISAC particle radius (in pixel).

        shift_rng_x (int): Translational search range in x-dimension.

        shift_rng_y (int): Translational search range in y-dimension.

        cuda_device_id (int): ID value of the GPU we are running the fit on.

        cuda_device_occ (float): Value in [0,1] that determines the percentage
            of GPU memory we would like to make use of. In general one does not
            want to use 100% of GPU memory since this might overwrite OS memory
            (if there is only a single GPU in use) or any other data structures
            that can reside on the card (e.g. CUDA libraries sometimes allocate
            additional work space for themselves).

        verbose (bool): Flag to print out additional information of the data
            fit. Primarily used for debugging and b/c it's interesting.

    Returns: 
        No. of images (first) and No. of classes (second) we can process on the
        card in parallel.
    """

    # alignment configuration
    aln_cfg = AlignConfig( 
        0, 0, img_dim,                # No. of images, No. of references, and image size
        outer_polar_radius, 256,      # polar sampling parameters (No. of rings, No. of ring elements)
        1, shift_rng_x, shift_rng_y,  # shift parameters (step, shift ranges in x/y dimension)
        "multigroup_aln".encode("utf-8"))
    aln_cfg.freeze()
    ...
    for split in [ 2**i for i in range(int(math.log(max_class_num,2))+1) ][::-1]:
        ...
        if cu_module.ref_free_alignment_2D_size_check( ctypes.byref(aln_cfg), ctypes.c_uint(cuda_device_id), ctypes.c_float(cuda_device_occ), ctypes.c_bool(False) ) == True:
            gpu_class_limit += split
    # NOTE: this is capped at the given maximum no. of classes, so it's NOT the
    # max load we can fit on the card, but the max load to fit all particles

    if verbose:
        print( "GPU fit: %d images over %d classes using" % (aln_cfg.sbj_num, aln_cfg.ref_num) )
        cu_module.ref_free_alignment_2D_size_check( ctypes.byref(aln_cfg), ctypes.c_uint(cuda_device_id), ctypes.c_float(cuda_device_occ), ctypes.c_bool(True) )

    # report max No. of full-sized classes we can process on a single gpu 
    return aln_cfg.sbj_num, aln_cfg.ref_num

def multigroup_refinement_gpu(
    sbj_img_list,
    sbj_cid_list,
    outer_polar_radius,
    shift_rng_x,
    ...
    """
    Reference free alignment of multiple, pre-defined classes.
    """

    # sanity check
    assert sbj_img_list[0].get_xsize() == sbj_img_list[0].get_ysize(), \
    "ASSERTION ERROR in multigroup_refinement_gpu(): Images are not square."

    #----------------------------------------------[ initialize gpu resources ]

    # alignment parameters
    aln_cfg = AlignConfig( 
        len(sbj_img_list),            # no. of images
        sbj_cid_list[-1]+1,           # no. of references
        sbj_img_list[0].get_xsize(),  # image size (this assumes images are square!)
        outer_polar_radius, 256,      # polar sampling parameters (no. of rings, no. of ring elements)
    ...
    cid_idx = numpy.append( numpy.where(numpy.roll(sbj_cid_list, 1) != sbj_cid_list)[0], len(sbj_cid_list) )
    if len(cid_idx) == 1: cid_idx = [0, cid_idx[0]]
    assert cid_idx[0] != cid_idx[-1] # sanity check

    # randomize alignment angles (in ISAC this flag is hardcoded and always True)
    if randomize:
        for sbj_img in sbj_img_list:
            angle, shift_x, shift_y, _, _   = sp_utilities.get_params2D( sbj_img )
            _,     shift_x, shift_y, _      = sp_utilities.inverse_transform2( angle, shift_x, shift_y )
            angle, shift_x, shift_y, mirror = sp_utilities.combine_params2( 0.0, -shift_x, -shift_y, 0, random.random()*360.0, 0.0, 0.0, random.randint(0,1) )
            sp_utilities.set_params2D( sbj_img, [angle, shift_x, shift_y, mirror, 1.0] )

    # build the initial averages (takes into account any existing shifts)
    avg_img_list = [ sp_statistics.ave_series( sbj_img_list[cid_idx[k]:cid_idx[k+1]] ) for k in range( len(cid_idx)-1 ) ]

    # initialize gpu resources (returns location for our alignment parameters in CUDA unified memory)
    aln_param = cu_module.ref_free_alignment_2D_init(
        ctypes.byref(aln_cfg),                             # C pointer to alignment parameters
        get_c_ptr_array(sbj_img_list),                     # C array holding pointers to particle image data
        get_c_ptr_array(avg_img_list),                     # C array holding pointers to initial references
        (ctypes.c_int*len(sbj_img_list))(*sbj_cid_list),   # class id (cid) list as C array
        ctypes.c_int(cuda_device_id))
    aln_param = ctypes.cast( aln_param, aln_param_ptr )    # cast to allow Python-side r/w access

    for sbj_idx, sbj_img in enumerate(sbj_img_list):
        angle, shift_x, shift_y, mirror, _ = sp_utilities.get_params2D( sbj_img )
        angle, shift_x, shift_y, _ = sp_utilities.inverse_transform2( angle, shift_x, shift_y )
        aln_param[sbj_idx].shift_x = shift_x
        aln_param[sbj_idx].shift_y = shift_y
    ...
    for i in range(num_iter):
        ...
        if cutoff_freq > 0.0:
            ...
            cu_module.ref_free_alignment_2D_filter_references( ctypes.c_float(freq), ctypes.c_float(falloff) )

        # run refinement iteration on the GPU
        cu_module.ref_free_alignment_2D()

        # progress bar
        done = int( float(i+1)/num_iter*50.0 )
        sys.stdout.write( "\r[GPU"+str(cuda_device_id)+"][ITER][" + "="*done + "-"*(50-done) + "]~[%d/%.2f%%]" % (i+1, (float(i+1)/num_iter)*100.0) )
        sys.stdout.flush()
    print()
    sys.stdout.write( "[GPU"+str(cuda_device_id)+"][ITER]["+"="*50+"]~[%d/%.2f%%]\n" % (num_iter, 100.0) )

    #-------------------------------------------[ export alignment parameters ]

    # convert alignment parameters to the format by ave_series() (CPU ISAC does this at the end of ormq)
    for k, img in enumerate(sbj_img_list):
        angle   =  aln_param[k].angle
        sx_neg  = -aln_param[k].shift_x
        sy_neg  = -aln_param[k].shift_y
        c_ang   =  math.cos( math.radians(angle) )
        s_ang   = -math.sin( math.radians(angle) )
        shift_x =  sx_neg*c_ang - sy_neg*s_ang
        shift_y =  sx_neg*s_ang + sy_neg*c_ang
        sp_utilities.set_params2D( img, [angle, shift_x, shift_y, aln_param[k].mirror, 1.0], "xform.align2d" )
