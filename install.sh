...
cp ./cuda/gpu_aln_pack.so $CONDA_PREFIX/lib/
...
echo -e "Installing GPU ISAC to conda environment.."
cp bin/sp_isac_applications.py $(dirname $(which sphire))/sp_isac_applications.py
cp bin/sp_isac_alignment.py    $(dirname $(which sphire))/sp_isac_alignment.py
cp bin/sp_isac2_gpu.py         $(dirname $(which sphire))/sp_isac2_gpu.py
